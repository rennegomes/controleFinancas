    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed p-4 bg-[var(--cor-destaque)] rounded-t-3xl pt-5 left-0 right-0 max-sm:bottom-0 md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-md md:max-h-[85vh] md:w-[90vw] md:max-w-[500px]">
        ...
        <form
          ...
          onSubmit={handleSubmit(acaoCriaNovaTransacao)}
        >
          <input
            type="text"
          ...
          <Controller
            ...
            render={({ field }) => {
              return (
                <RadioGroup.Root
                  className="group grid grid-cols-2 gap-5"
                  onValueChange={field.onChange}
                  value={field.value}
                >
                  <RadioGroup.Item
                    value="entrada"
                    className="data-[state=checked]:bg-[var(--cor-positiva-verde-escuro)] data-[state=checked]:text-[var(--cor-texto-padrao)] flex gap-2 justify-center bg-[var(--cor-off-destaque)] rounded-md p-4 mt-5 cursor-pointer text-[var(--cor-positiva-verde)]"
                  >
                    <ArrowCircleUpIcon size={24} />
                    <span className="text-[var(--cor-texto-padrao)]">
                      Entrada
                    </span>
                  </RadioGroup.Item>
                  <RadioGroup.Item
                    value="saida"
                    className="data-[state=checked]:bg-[var(--cor-negativa-vermelho-escuro)] data-[state=checked]:text-[var(--cor-texto-padrao)] flex gap-2 justify-center bg-[var(--cor-off-destaque)] rounded-md p-4 mt-5 cursor-pointer text-[var(--cor-negativa-vermelho)]"
                  >
